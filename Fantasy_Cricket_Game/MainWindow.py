...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        #DOUBLE CLICK
        self.listWidget_2.itemDoubleClicked.connect(self.list1)
        self.listWidget.itemDoubleClicked.connect(self.list2)
...
##########################################################################################
    ...
    def error(self):
        ...
        if self.batsmancount>=0 and self.batsmancount<6:
            ...
            error_dialog.showMessage('Please Select Only 5 batsman')
            error_dialog.exec_()

        if self.wkeepercount>=0 and self.wkeepercount<2:
            pass
        else:
            error_dialog.showMessage('Only 1 keeper are allowed')
            error_dialog.exec_()

        if self.ballcount>=0 and self.ballcount<5:
            pass
        else:
            error_dialog.showMessage('Please Select Only 5 bowlers')
            error_dialog.exec_()

        if self.totalcount>=0 and self.totalcount<12:
            pass
        else:
            error_dialog.showMessage('No more than 11 Players are allowed')
            error_dialog.exec_()
